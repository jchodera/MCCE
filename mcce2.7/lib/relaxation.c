...
int relaxation(PROT prot) {
    ...
    while (n_pair_relaxed) {
            ...
            for (j_res=0; j_res<prot.n_res; j_res++) {
                ...
                for (i_conf=0; i_conf<prot.res[i_res].n_conf; i_conf++) {
                    ...
                    for (j_conf=0; j_conf<prot.res[j_res].n_conf; j_conf++) {
                        ...
                        if ((!prot.res[i_res].relax||!i_conf) && (!prot.res[j_res].relax||!j_conf)) continue; /* both conformers are not relaxable */
                        if (i_res == j_res && (i_conf*j_conf != 0 && i_conf != j_conf)) continue; /* Same residue, two different sidechain */
                        ...
                        if (pair_vdw < env.hv_relax_vdw_thr) continue;
        ...
        if (progress_fp) {
            fprintf(progress_fp, "   Relaxation Cycle = %3d, n_relaxed = %5d, shake convergence rate = %6.2f %%\n",i_cycle,n_pair_relaxed, 100.*(float)n_pair_converged/(float)n_pair_trial); fflush(stdout);
...
void relaxation_setup(PROT prot) {
    ...
    for (ia=0;ia<na;ia++) {
        ...
        all_atoms[ia].constr_dsq  = malloc(all_atoms[ia].n_constr * sizeof(float));
...
void get_frc(float tors_scale, PROT prot) {
    ...
    for (i_relax=0;i_relax<n_relax;i_relax++) {
        ...
        printf("ATOM        %4s%c%3s %c%04d%c%03d%8.3f%8.3f%8.3f %8.3f%8.3f%8.3f\n",
        prot.res[i_res].conf[i_conf].atom[i_atom].name,
        prot.res[i_res].conf[i_conf].altLoc,
        prot.res[i_res].resName,
        prot.res[i_res].chainID,
        prot.res[i_res].resSeq,
        prot.res[i_res].iCode,
        i_conf,
        prot.res[i_res].conf[i_conf].atom[i_atom].xyz.x,
        prot.res[i_res].conf[i_conf].atom[i_atom].xyz.y,
        prot.res[i_res].conf[i_conf].atom[i_atom].xyz.z,
    ...
    for (i_relax=0;i_relax<n_relax;i_relax++) {
        ...
        for (i_ngh=0; i_ngh<relax_atoms[i_relax].n_ngh; i_ngh++) {
            C6 = C6_matrix[relax_atoms[i_relax].atom_p->i_elem][relax_atoms[i_relax].ngh[i_ngh]->atom_p->i_elem];
            C12 = C12_matrix[relax_atoms[i_relax].atom_p->i_elem][relax_atoms[i_relax].ngh[i_ngh]->atom_p->i_elem];
        ...
        if (relax_atoms[i_relax].atom_p->crg > 1e-6 || relax_atoms[i_relax].atom_p->crg < -1e-6) {
            for (i_ngh=0; i_ngh<relax_atoms[i_relax].n_ngh; i_ngh++) {
                if (relax_atoms[i_relax].ngh[i_ngh]->atom_p->crg < 1e-6
                    && relax_atoms[i_relax].ngh[i_ngh]->atom_p->crg > -1e-6) continue;
            ...
            for (i_ngh=0; i_ngh<relax_atoms[i_relax].n_ngh14; i_ngh++) {
                if (relax_atoms[i_relax].ngh14[i_ngh]->atom_p->crg < 1e-6
                    && relax_atoms[i_relax].ngh14[i_ngh]->atom_p->crg > -1e-6) continue;
                relax_atoms[i_relax].elec_frc = vector_vplusv(relax_atoms[i_relax].elec_frc,
                coulomb_frc(relax_atoms[i_relax].r,
        ...
        if (relax_atoms[i_relax].tor_atom1 != -1) {
            ...
            atom1_r = relax_atoms[all_atoms[relax_atoms[i_relax].tor_atom1].i_relax].r;
            atom2_r = relax_atoms[all_atoms[relax_atoms[i_relax].tor_atom2].i_relax].r;
            atom3_r = relax_atoms[all_atoms[relax_atoms[i_relax].tor_atom3].i_relax].r;
            ...
            for (i_rotate = 0; i_rotate < relax_atoms[i_relax].n_rotate1; i_rotate++) {
                int k_relax = all_atoms[relax_atoms[i_relax].rotate1_lst[i_rotate]].i_relax;
            ...
            for (i_rotate = 0; i_rotate < relax_atoms[i_relax].n_rotate2; i_rotate++) {
                int k_relax = all_atoms[relax_atoms[i_relax].rotate2_lst[i_rotate]].i_relax;
                if (k_relax == -1) continue;
                r = vector_vminusv(relax_atoms[k_relax].r, atom1_r);
                r_p = vector_vminusv(r, vector_rescale(k,vdotv(r,k)));
...
int shake() {
    ...
    int done;
    float TOL2 = env.hv_relax_shake_tol * 2.;
    ...
    while (iter) {
        ...
        for (i_relax =0; i_relax<n_relax; i_relax++) {
            for (i_constr=0; i_constr<relax_atoms[i_relax].n_constr; i_constr++) {
                if (!relax_atoms[i_relax].moved && !relax_atoms[i_relax].constr_list[i_constr]->moved) continue;
...
void complete_constr(int i_res, int i_conf, int j_res, int j_conf) {
    int i_relax, i_constr, ia, ja, adding, i_rotate;
    adding = 1;
    while (adding) {
        adding = 0;
        /* add all atoms in torsion list into relaxation list */
        for (i_relax =0; i_relax<n_relax; i_relax++) {
            if (!relax_atoms[i_relax].movable) continue;
            ia = relax_atoms[i_relax].atom_p->i_atom_prot;
            if (relax_atoms[i_relax].tor_atom1 == -1) continue;
            
            ja = relax_atoms[i_relax].tor_atom1;
            if (!res_in_relax(all_atoms[ja].atom_p->i_res_prot)) {
                if (!in_relax_list(ja)) { /* if ja is not in the relaxed atoms */
                    add_atom2relax(ja, 1);
                    adding = 1;
                }
            }
        ...
        /* add all atoms in constraint list into relaxation list */
        for (i_relax =0; i_relax<n_relax; i_relax++) {
            if (!relax_atoms[i_relax].movable) continue;
            ia = relax_atoms[i_relax].atom_p->i_atom_prot;
            for (i_constr=0; i_constr<all_atoms[ia].n_constr; i_constr++) {
                
                ja = all_atoms[ia].constr_list[i_constr]->atom_p->i_atom_prot;
                if (!res_in_relax(all_atoms[ja].atom_p->i_res_prot)) {
                    if (!in_relax_list(ja)) { /* if ja is not in the relaxed atoms */
                        add_atom2relax(ja, 1);
                        adding = 1;
    ...
    /* check if all tors atoms in the relax list */
    for (i_relax =0; i_relax<n_relax; i_relax++) {
        if (relax_atoms[i_relax].tor_atom1 == -1) continue;
        ...
        //printf("%s %d %d %d\n",relax_atoms[i_relax].atom_p->name,relax_atoms[i_relax].tor_atom1,relax_atoms[i_relax].tor_atom2,relax_atoms[i_relax].tor_atom3);
    }
}
...
int in_relax_list(int ia) {
    ...
    for (i_relax = 0; i_relax < n_relax; i_relax++) {
        if (all_atoms[ia].atom_p == relax_atoms[i_relax].atom_p) break;
    }
    if (i_relax < n_relax) return 1;
...
void setup_nghlst(PROT prot) {
    ...
    for (i_relax =0; i_relax<n_relax; i_relax++) {
        ...
        for (i_constr=0; i_constr<all_atoms[ia].n_constr; i_constr++) {
            for (j_relax =0; j_relax<n_relax; j_relax++) {
                //if (i_relax == j_relax) continue;
                if (relax_atoms[j_relax].atom_p == all_atoms[ia].constr_list[i_constr]->atom_p) {
                    break;
                }
            }
            if (j_relax<n_relax) {
                ...
                relax_atoms[i_relax].constr_dsq = realloc(relax_atoms[i_relax].constr_dsq, relax_atoms[i_relax].n_constr*sizeof(float));
        ...
        for (i_connect=0; i_connect < n_connect14; i_connect++) {
            for (j_relax =0; j_relax<n_relax; j_relax++) {
                if (relax_atoms[j_relax].atom_p == prot.res[i_res].connect14[i_conf][i_atom][i_connect]) {
                    break;
                }
            }
            if (j_relax<n_relax) {
