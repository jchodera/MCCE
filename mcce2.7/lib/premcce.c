...
#define FLOATING "HOH NO3 PO4 SO4 K NA CL OH BR"
...
{  FILE *fp1, *fp2, *fp;
    ...
    */
    ...
    /* cutoff water */
    printf("   Strip water molecules with SAS > %3.f%%...\n", env.h2o_sascutoff*100);
    fflush(stdout);
    assign_rad(prot);
    Counter = 1; Nhoh = 0;
    while (Counter) {
       ...
       Nhoh += Counter;
       printf("      %3d water/salt molecules were stripped off in this round\n", Counter);
    }
    print_surfw(prot);

    printf("   Total deleted HOH = %d.\n", Nhoh);
    printf("   Done\n\n");
    fflush(stdout);
    ...
    while ( fgets(sbuffer, MAXCHAR_LINE, fp) ) {
        ...
        if (!strncmp("ATOM  ", sbuffer, 6) || !strncmp("HETATM", sbuffer, 6)) {
            ...
            if ( !param_get("IATOM", conflist.strings[0], atom.name, &iatom) ) {
                strncpy(pdb.strings[i-1]+80, "BK", 2);
...
{  int i;
   ...
   for (i=0; i<prot->n_res; i++) {
      if (strstr(FLOATING, prot->res[i].resName) && prot->res[i].sas > env.h2o_sascutoff) {
