...
RESIDUE  *free_res, *fixed_res, *all_res;
int      n_free, n_fixed, n_all;
...
{  int i, j, k, counter;
    ...
    for (i=0; i<env.titr_steps; i++) {
        ...
        */
        ...
        if (env.monte_tsx) {
           while ((S_max > 0.5) && (j < Max_TSsampling)) {
             E_base = get_base();
...
{  int i, j;
    ...
    Counter = 0;
    for (i=0; i <conflist.n_conf; i++) {
        if (conflist.conf[i].uniqID[3] == '+' || conflist.conf[i].uniqID[3] == '-') {
    ...
    for(i=0; i<N_res; i++) {
        ...
        /* number of protons on excited conformer type */
        ...
        if (param_get( "PROTON", sbuff, "", &n_protons_grnd)) n_protons = 0;
        if (param_get( "ELECTRON", sbuff, "", &n_electrons_grnd)) n_electrons = 0;
    ...
    fprintf(fp, "       pKa/Em  n(slope) 1000*chi2\n");
    for (i=0; i <N_res; i++) {
        ...
        if (mid >= 0.485) {
            if (fabs(yp[0]-yp[Nx-1])>0.5) {  /* jumps form <0.015 to >0.985 */
               fprintf(fp, "%s          pKa titration curve too sharp\n", shead[i]);
            }
        ...
        else fprintf(fp, "%s    %9.3f %9.3f %9.3f\n", shead[i], stat.b, n, 1000*stat.chi2);
    }
...
{  int i;
   ...
   for (i=start; i<end; i++) {
      if (conflist.conf[i].occ/sum > 0.0001)
